// *******Credit for Parts of the Vector classes and Matrix Code goes to Cherno and were based off his Sparky Game Engine code.
...
Vec2& Vec2::operator +=(const float &other){return add(other);}
Vec2& Vec2::operator -=(const float &other){return subtract(other);}
Vec2& Vec2::operator *=(const float &other){return multiply(other);}
Vec2& Vec2::operator /=(const float &other){return divide(other);}
...
{
    Print("This is currently Not Finished Vec3::RayCast LINE: 340 Vertex.Cpp");
    float Offset = 90;
...
Matrix3& Matrix3::Multiply(const Matrix3& other)
{
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            for (int k = 0; k < 3; ++k) {
                temp.elements[i + 3 * j] += (elements[i + 3 * k] * other.elements[k + 3 * j]);
            }
        }
    }
return (*this = temp);
...
Matrix3 operator *(Matrix3 left, Matrix3 const &right)
{
	return left.Multiply(right);
}
